#!/bin/sh
#------------------------------------------------
# The Virtual Monte Carlo examples
# Copyright (C) 2007 - 2014 Ivana Hrivnacova
# All rights reserved.
#
# For the licensing terms see geant4_vmc/LICENSE.
# Contact: root-vmc@cern.ch
#-------------------------------------------------

#
# Run tests for all VMC examples and regenerate output files
# Usage:
# test_suite.sh [--g3=on|off] [--g4=on|off][--builddir=dir]
#
# By I. Hrivnacova, IPN Orsay

CURDIR=`pwd`
OUTDIR=$CURDIR/log/test_suite

TMPDIR="/tmp/GEANT4_VMC"

PASSED="0"
FAILED="0"
# If that's ==0 means not in test
CURRENT_TEST="0"
CURRENT_LOG=""
CURRENT_TEST_CASE="0"


# Set 1 to 0 if you want to skip given MC or ExGarfield test
TESTG3="1"
TESTG4="1"
BUILDDIR=""

# Run Garfield only with Root 5
TESTGARFIELD="1"
ROOT_VERSION=`root-config --version`
if [ ${ROOT_VERSION:0:1} = "6" ]; then
  TESTGARFIELD="0"
fi

# Root command with loading g3/g4 libraries
RUNG3="root.exe -b -q load_g3.C"
RUNG4="root.exe -b -q load_g4.C"

EXAMPLES="E01 E02 E03 E06 A01 ExGarfield Gflash TR EME"
USER_EXAMPLES=""

STREAM_START_RED="\033[0;31m"
STREAM_START_GREEN="\033[0;32m"
STREAM_END_COLOR="\033[0m"

VERBOSE_SCRIPT="0"
VERBOSE_MACROS="0"

function start_test()
{
  [[ "$CURRENT_TEST" != "0" ]] && { echo "Currently in test $CURRENT_TEST which needs to be finished first. Check this script and call \"finish_test\" first": exit 1; }
  local user_message="$1"
  CURRENT_TEST=`expr $FAILED + $PASSED + 1`
  CURRENT_LOG=$TMPDIR/test_$CURRENT_TEST.out
  touch $CURRENT_LOG

  echo "#######################"
  echo "---> START test $CURRENT_TEST"
  echo "$user_message"
}

function run_test_case()
{
  if [[ "CURRENT_TEST" == "0" ]]
  then
    echo "ERROR: Not in test. Call \"start_test\" first"
  else
    if [[ "$#" != "1" ]]
    then
      echo "Exactly 1 argument requried for run_test_case(), namely a run string"
    else
      local run_string="$1"
      local log_file_tmp=$TMPDIR/test_${CURRENT_TEST}_tmp.out
      CURRENT_TEST_CASE=`expr $CURRENT_TEST_CASE + 1`
      echo "Test case $CURRENT_TEST_CASE"
      echo "Run command: $run_string"
      $run_string &> $log_file_tmp
      CURRENT_TEST_STATUS="$?"
      cat $log_file_tmp >> $CURRENT_LOG
    fi
  fi
}

function write_log()
{
  local log_file="$1"
  if [[ "$log_file" != "" && "$CURRENT_LOG" != "" ]]
  then
    cp $CURRENT_LOG $log_file
    echo "Log file written to $log_file"
  fi
}

function finish_test()
{
  local log_file="$1"
  if [ "$CURRENT_TEST_STATUS" -ne "0" ]
  then
    FAILED=`expr $FAILED + 1`
    echo -e "${STREAM_START_RED}failed${STREAM_END_COLOR}"
  else
    PASSED=`expr $PASSED + 1`
    echo -e "${STREAM_START_GREEN}passed${STREAM_END_COLOR}"
  fi

  # Write log if requested
  [[ "$log_file" != "" ]] && write_log $log_file
  CURRENT_TEST="0"
  CURRENT_LOG=""
  CURRENT_TEST_CASE="0"
}



function print_help()
{
  echo "Usage:"
  echo "test_suite.sh [--g3 on|off] [--g4 on|off] [--builddir dir]"
}

# Process script arguments
for arg in "${@}"
do
  #echo "got: $arg"
  case $arg in
    "--g3=on"  ) TESTG3="1" ;;
    "--g3=off" ) TESTG3="0" ;;
    "--g4=on"  ) TESTG4="1" ;;
    "--g4=off" ) TESTG4="0" ;;
    "--garfield=on"  ) TESTGARFIELD="1" ;;
    "--garfield=off" ) TESTGARFIELD="0" ;;
     --examples=* ) USER_EXAMPLES=${arg#--examples=} ;;
     --builddir=* ) BUILDDIR=${arg#--builddir=} ;;
    * ) echo "Unsupported option $arg chosen."
        print_help
        exit 1
        ;;
  esac
done

[[ "$USER_EXAMPLES" != "" ]] && EXAMPLES="$USER_EXAMPLES"

# Recreate log directory only if running test for both G3 and G4
if [ "$TESTG3" = "1" -a  "$TESTG4" = "1" ]; then
  rm -fr $OUTDIR
fi

# Set path to shared libraries if --builddir is provided via the option
if [ "x${BUILDDIR}" != "x" ]; then
  LIBS_FROM_BUILDDIR=$(find ${BUILDDIR} -iname "*.so" -exec dirname {} \; | tr '\r\n' ':')
  export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:${LIBS_FROM_BUILDDIR}
fi

# Create tmp dir from scratch
rm -rf $TMPDIR &> /dev/null
mkdir $TMPDIR

for EXAMPLE in $EXAMPLES
do
  OUT=$OUTDIR/$EXAMPLE
  if [ ! -d $OUT ]; then
    mkdir -p $OUT
  fi

  # skip ExGarfield if switch off
  if [ "$EXAMPLE" = "ExGarfield" -a "$TESTGARFIELD" = "0" ]; then
    continue 1
  fi

  cd $CURDIR/$EXAMPLE

  echo "... Example $EXAMPLE"

  if [ "$EXAMPLE" = "E01" -o "$EXAMPLE" = "E02" -o "$EXAMPLE" = "E06" ]; then

    if [ "$TESTG3" = "1" ]; then
      start_test "... Running test with G3, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG3 test_$EXAMPLE.C(\"g3tgeoConfig.C\",kFALSE)"
      # stack popper test
      if [ "$EXAMPLE" = "E06" ]; then
        #echo "... Running test with G3, geometry via TGeo, TGeo navigation, with Feedback photons"
        run_test_case "$RUNG3 test_E06_2.C(\"g3tgeoConfig.C\",kFALSE)"
      fi
      finish_test "$OUT/test_g3_tgeo_tgeo.out"

      start_test "... Running test with G3, geometry via VMC,  Native navigation"
      run_test_case "$RUNG3 test_$EXAMPLE.C(\"g3Config.C\",kTRUE)"
      finish_test "$OUT/test_g3_vmc_nat.out"

      start_test "... Running test with G3, geometry via VMC,  TGeo navigation"
      run_test_case "$RUNG3 test_$EXAMPLE.C(\"g3tgeoConfig.C\",kTRUE)"
      finish_test "$OUT/test_g3_vmc_tgeo.out"
    fi

    if [ "$TESTG4" = "1" ]; then
      start_test "... Running test with G4, geometry via TGeo, Native navigation"
      run_test_case "$RUNG4 test_$EXAMPLE.C(\"g4Config.C\",kFALSE)"
      # stack popper test
      if [ "$EXAMPLE" = "E06" ]; then
        #echo "... Running test with G4, geometry via TGeo, Native navigation, with stackPopper"
        run_test_case "$RUNG4 test_E06_2.C(\"g4Config1.C\",kFALSE)"
      fi
      finish_test "$OUT/test_g4_tgeo_nat.out"

      start_test "... Running test with G4, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_$EXAMPLE.C(\"g4tgeoConfig.C\",kFALSE)"
      # stack popper test
      if [ "$EXAMPLE" = "E06" ]; then
        #echo "... Running test with G4, geometry via TGeo, TGeo navigation, with stackPopper"
        run_test_case "$RUNG4 test_E06_2.C(\"g4tgeoConfig1.C\",kFALSE)"
      fi
      finish_test "$OUT/test_g4_tgeo_tgeo.out"

      start_test "... Running test with G4, geometry via VMC, Native navigation"
      run_test_case "$RUNG4 test_$EXAMPLE.C(\"g4ConfigOld.C\",kTRUE)"
      finish_test "$OUT/test_g4_vmc_nat.out"

      start_test "... Running test with G4, geometry via VMC, TGeo navigation"
      #echo $RUNG4 "test_$EXAMPLE.C(\"g4tgeoConfigOld.C\",kTRUE)" >& $OUT/test_g4_vmc_tgeo.out
      run_test_case "$RUNG4 test_$EXAMPLE.C(\"g4tgeoConfigOld.C\",kTRUE)"
      finish_test "$OUT/test_g4_vmc_tgeo.out"
    fi
  fi

  if [ "$EXAMPLE" = "E03" ]; then
    # Run three macro + special configuration available only in E03 example
    if [ "$TESTG3" = "1" ]; then
      start_test "... Running test with G3, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG3 test_E03_1.C(\"g3tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG3 test_E03_2.C(\"g3tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG3 test_E03_3.C(\"g3tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG3 test_E03_4.C(\"g3tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG3 test_E03_5.C(\"g3tgeoConfig.C\",kFALSE)"
      finish_test "$OUT/test_g3_tgeo_tgeo.out"

      start_test "... Running test with G3, geometry via VMC, Native navigation"
      run_test_case "$RUNG3 test_E03_1.C(\"g3Config.C\",kTRUE)"
      run_test_case "$RUNG3 test_E03_2.C(\"g3Config.C\",kTRUE)"
      run_test_case "$RUNG3 test_E03_3.C(\"g3Config.C\",kTRUE)"
      run_test_case "$RUNG3 test_E03_4.C(\"g3Config.C\",kTRUE)"
      run_test_case "$RUNG3 test_E03_5.C(\"g3Config.C\",kTRUE)"
      finish_test "$OUT/test_g3_vmc_nat.out"

      start_test "... Running test with G3, geometry via VMC, TGeo navigation"
      run_test_case "$RUNG3 test_E03_1.C(\"g3tgeoConfig.C\",kTRUE)"
      run_test_case "$RUNG3 test_E03_2.C(\"g3tgeoConfig.C\",kTRUE)"
      run_test_case "$RUNG3 test_E03_3.C(\"g3tgeoConfig.C\",kTRUE)"
      run_test_case "$RUNG3 test_E03_4.C(\"g3tgeoConfig.C\",kTRUE)"
      run_test_case "$RUNG3 test_E03_5.C(\"g3tgeoConfig.C\",kTRUE)"
      finish_test "$OUT/test_g3_vmc_tgeo.out"
    fi

    if [ "$TESTG4" = "1" ]; then
      start_test "... Running test with G4, geometry via TGeo, Native navigation"
      run_test_case "$RUNG4 test_E03_1.C(\"g4Config.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_2.C(\"g4Config.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_3.C(\"g4Config.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_4.C(\"g4Config.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_5.C(\"g4Config4.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_6.C(\"g4Config5.C\",kFALSE)"
      finish_test "$OUT/test_g4_tgeo_nat.out"

      start_test "... Running test with G4, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_E03_1.C(\"g4tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_2.C(\"g4tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_3.C(\"g4tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_4.C(\"g4tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_5.C(\"g4tgeoConfig4.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_6.C(\"g4tgeoConfig5.C\",kFALSE)"
      finish_test "$OUT/test_g4_tgeo_tgeo.out"

      start_test "... Running test with G4, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_E03_1.C(\"g4ConfigOld.C\",kTRUE)"
      run_test_case "$RUNG4 test_E03_2.C(\"g4ConfigOld.C\",kTRUE)"
      run_test_case "$RUNG4 test_E03_3.C(\"g4ConfigOld.C\",kTRUE)"
      run_test_case "$RUNG4 test_E03_4.C(\"g4ConfigOld.C\",kTRUE)"
      finish_test "$OUT/test_g4_vmc_nat.out"

      start_test "... Running test with G4, geometry via VMC, TGeo navigation"
      run_test_case "$RUNG4 test_E03_1.C(\"g4tgeoConfigOld.C\",kTRUE)"
      run_test_case "$RUNG4 test_E03_2.C(\"g4tgeoConfigOld.C\",kTRUE)"
      run_test_case "$RUNG4 test_E03_3.C(\"g4tgeoConfigOld.C\",kTRUE)"
      run_test_case "$RUNG4 test_E03_4.C(\"g4tgeoConfigOld.C\",kTRUE)"
      finish_test "$OUT/test_g4_vmc_tgeo.out"

      start_test "... Running test with G4, geometry via G4, Native navigation"
      run_test_case "$RUNG4 test_E03_1.C(\"g4Config1.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_2.C(\"g4Config1.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_3.C(\"g4Config1.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_4.C(\"g4Config1.C\",kFALSE)"
      finish_test "$OUT/test_g4_g4_nat.out"

      start_test "... Running test with G4, geometry via TGeo, Native navigation, User physics list"
      run_test_case "$RUNG4 test_E03_1.C(\"g4Config2.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_2.C(\"g4Config2.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_3.C(\"g4Config2.C\",kFALSE)"
      run_test_case "$RUNG4 test_E03_4.C(\"g4Config2.C\",kFALSE)"
      finish_test "$OUT/test_g4_tgeo_nat_pl.out"
    fi
  fi

  if [ "$EXAMPLE" = "A01" ]; then
    if [ "$TESTG3" = "1" ]; then
      start_test "... Running test with G3, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG3 test_A01_1.C(\"g3tgeoConfig.C\",kFALSE)"
      finish_test "$OUT/test_g3_tgeo_tgeo.out"
    fi

    if [ "$TESTG4" = "1" ]; then
      start_test "... Running test with G4, geometry via TGeo, Native navigation"
      run_test_case "$RUNG4 test_A01_1.C(\"g4Config.C\",kFALSE)"
      run_test_case "$RUNG4 test_A01_2.C(\"g4Config2.C\",kFALSE)"
      finish_test "$OUT/test_g4_tgeo_nat.out"

      start_test "... Running test with G4, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_A01_1.C(\"g4tgeoConfig.C\",kFALSE)"
      run_test_case "$RUNG4 test_A01_2.C(\"g4tgeoConfig2.C\",kFALSE)"
      finish_test "$OUT/test_g4_tgeo_tgeo.out"

      start_test "... Running test with G4, geometry via G4, Native navigation"
      run_test_case "$RUNG4 test_A01_1.C(\"g4Config1.C\",kFALSE)"
      run_test_case "$RUNG4 test_A01_2.C(\"g4Config1.C\",kFALSE)"
      finish_test "$OUT/test_g4_g4_nat.out"
    fi
  fi

  # new examples (without old geometry definition)
  #
  if [ "$EXAMPLE" = "ExGarfield" -o "$EXAMPLE" = "Gflash"  -o "$EXAMPLE" = "TR" ]; then

    if [ "$TESTG3" = "1" ]; then
      start_test "... Running test with G3, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG3 test_$EXAMPLE.C(\"g3tgeoConfig.C\")"
      finish_test "$OUT/test_g3_tgeo_tgeo.out"
    fi

    if [ "$TESTG4" = "1" ]; then
      start_test "... Running test with G4, geometry via TGeo, Native navigation"
      run_test_case "$RUNG4 test_$EXAMPLE.C(\"g4Config.C\")"
      finish_test "$OUT/test_g4_tgeo_nat.out"

      start_test "... Running test with G4, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_$EXAMPLE.C(\"g4tgeoConfig.C\")"
      finish_test "$OUT/test_g4_tgeo_tgeo.out"
    fi
  fi

  # new examples using only TGeo for both construction and navigation
  #
  if [ "$EXAMPLE" = "EME" ]; then

    if [ "$TESTG3" = "1" ]; then
      start_test "... Running test with G3, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG3 test_${EXAMPLE}_single.C(\"g3tgeoConfig.C\")"
      finish_test "$OUT/test_g3_tgeo_tgeo.out"
    fi

    if [ "$TESTG4" = "1" ]; then
      start_test "... Running test with G4, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_${EXAMPLE}_single.C(\"g4tgeoConfig.C\")"
      finish_test "$OUT/test_g4_tgeo_tgeo.out"
    fi

    if [ "$TESTG3" = "1" -a "$TESTG4" = "1" ]; then
      start_test "... Running test with mixed G3 and G4, but only use G4, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_${EXAMPLE}_multi.C(\"g3tgeoConfig.C\",\"g4tgeoConfig.C\")"
      finish_test "$OUT/test_g3g4_tgeo_tgeo.out"

      start_test "... Running test with mixed G4 and G3, but only use G3, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_${EXAMPLE}_multi.C(\"g4tgeoConfig.C\",\"g3tgeoConfig.C\")"
      finish_test "$OUT/test_g4g3_tgeo_tgeo.out"

      start_test "... Running test with mixed G3 and G4, split simulation dep. on geometry, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_${EXAMPLE}_multi_split.C(\"g3tgeoConfig.C\",\"g4tgeoConfig.C\",0)"
      finish_test "$OUT/test_g3g4_split_tgeo_tgeo.out"

      start_test "... Running test with mixed G4 and G3, split simulation dep. on geometry, geometry via TGeo, TGeo navigation"
      run_test_case "$RUNG4 test_${EXAMPLE}_multi_split.C(\"g4tgeoConfig.C\",\"g3tgeoConfig.C\",0)"
      finish_test "$OUT/test_g4g3_split_tgeo_tgeo.out"

      start_test "... Running test with mixed G3 and G4, split simulation dep. on geometry, geometry via TGeo, TGeo navigation, additional verbosity"
      run_test_case "$RUNG4 test_${EXAMPLE}_multi_split.C(\"g3tgeoConfig.C\",\"g4tgeoConfig.C\",1)"
      finish_test "$OUT/test_g3g4_split_tgeo_tgeo_verbose.out"

      start_test "... Running test with mixed G4 and G3, split simulation dep. on geometry, geometry via TGeo, TGeo navigation, additional verbosity"
      run_test_case "$RUNG4 test_${EXAMPLE}_multi_split.C(\"g4tgeoConfig.C\",\"g3tgeoConfig.C\",1)"
      finish_test "$OUT/test_g4g3_split_tgeo_tgeo_verbose.out"
    fi
  fi

  echo " "
done

# remove tmp dir
rm -rf $TMPDIR &> /dev/null

echo " "
echo "Test summary"
echo " "
# Print summary message
if [ "$FAILED" -eq "0" -a  "$PASSED" -ne "0" ]; then
  echo -e "... ${STREAM_START_GREEN}All ($PASSED) tests passed successfully.${STREAM_END_COLOR}"
elif [ "$FAILED" -ne "0" -a  "$PASSED" -eq "0" ]; then
  echo -e "... ${STREAM_START_RED}All ($FAILED) tests failed.${STREAM_END_COLOR}"
else
  echo -e "... ${STREAM_START_GREEN}$PASSED tests passed successfully.${STREAM_END_COLOR}"
  echo -e "... ${STREAM_START_RED}$FAILED tests failed.${STREAM_END_COLOR}"
fi
echo " "

cd $CURDIR

# Clean-up generated files
rm -f */Example*.root
rm -f */gphysi.dat

exit $FAILED
