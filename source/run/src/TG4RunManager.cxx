  ...
  if ( userGeometry == "VMCtoRoot" || userGeometry == "Root" ) {
    ...
    if(!fRunConfiguration->UseExternalGeometryConstruction()) {
      ...
      if ( TG4GeometryManager::Instance()->VerboseLevel() > 0 )
        G4cout << "Running TVirtualMCApplication::ConstructGeometry" << std::endl;
...
/// Cache the pointer to thread-local VMC stack
  ...
  // Having the TVirtualMCStack pointer is a must whereas the TMCManagerStack
  // is optional.
  if ( ! mcStack ) {
...
/// Not yet implemented.
  ...
  if(!fHasRunInitialization) {
    ...
    fHasRunInitialization = true;
  }
  fIsInterruptibleEvent = isInterruptible;
  GetEventAction()->SetInterruptibleEvent(isInterruptible);
