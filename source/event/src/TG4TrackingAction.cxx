...
/// Called by G4 kernel before starting tracking.
  ...
  if ( isFirstStep ) {
    ...
    if (track->GetParentID() == 0) {
      ...
      if(!particleStatus || (particleStatus->fCurrentStepNumber == 0 &&
                             particleStatus->fParentId < 0)) {
    ...
    if ( fTrackSaveControl == kSaveInPreTrack ) {
      ...
      // NOTE The particle Id has been set by fTrackManager->TrackToStack
      // /fMCStack->SetCurrentTrack(trackInfo->GetTrackParticleID());
  ...
  if(particleStatus) {
    fTrackManager->NotifyOnRestoredGeometry(track);
  }
}
...
/// Called by G4 kernel after finishing tracking.
  ...
  if ( track->GetTrackStatus() != fSuspend  &&
       !fTrackManager->GetTrackInformation(track)->IsInterrupt() ) {
...
// ---
  // NOTE Why is this not called from PostUserTrackingAction()?
